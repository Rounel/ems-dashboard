    ...
    <div className="space-y-5 p-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-500">Score ISO groupe</p>
          <p className={`mt-2 text-2xl font-bold ${lastScore >= lastObj ? 'text-emerald-600' : 'text-amber-600'}`}>
            {lastScore.toFixed(1)} / 100
          </p>
          <p className="mt-1 text-xs text-gray-500">Objectif : {lastObj}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-500">Indicateurs hors objectif</p>
          <p className={`mt-2 text-2xl font-bold ${sitesAbove === 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            {sitesAbove} / {ISO_COMPARISON.length}
          </p>
          <p className="mt-1 text-xs text-gray-500">dépassent l&apos;objectif</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-500">Tendance annuelle</p>
          <p className="mt-2 text-2xl font-bold text-blue-600">
            +{(ISO_ANNUAL[ISO_ANNUAL.length - 1].realise - ISO_ANNUAL[0].realise).toFixed(1)} pts
          </p>
          <p className="mt-1 text-xs text-gray-500">depuis Avr 2024</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white">
        <div className="border-b border-gray-200 px-5 py-3">
          <p className="text-sm font-semibold text-gray-900">Tableau comparatif inter-sites</p>
        </div>
              ...
              {ISO_COMPARISON.map((row, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-gray-100">
                    <td className="px-5 py-3 font-medium text-gray-900">{row.site}</td>
                    <td className="px-5 py-3 text-gray-600">{row.indicateur}</td>
                    <td className="px-5 py-3 text-gray-500">{row.unit}</td>
                    <td className="px-5 py-3 font-mono text-gray-700">{row.objectif}</td>
                    <td className="px-5 py-3 font-mono font-semibold text-gray-900">{row.realise}</td>
