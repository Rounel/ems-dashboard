  ...
  return (
    <div className="space-y-5 p-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Alertes actives',        v: active,        bg: active > 5 ? 'bg-amber-500' : 'bg-emerald-500' },
          { label: 'Critiques non traitées', v: critiques,     bg: critiques > 0 ? 'bg-red-500' : 'bg-emerald-500' },
          { label: 'Type Administrateur',    v: admin,         bg: admin > 5 ? 'bg-red-500' : 'bg-emerald-500'  },
          { label: 'Type Opérationnel',      v: operationnel,  bg: operationnel > 5 ? 'bg-red-500' : 'bg-emerald-500'},
        ].map(({ label, v, bg }) => (
